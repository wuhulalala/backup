...
def main():
  ...
  logging.info("Loaded the processor")
  ...
  dataset_vqa_train = load_vqa(train=False, local=args.local_datasets)
  ...
  num_calibration_samples = 32 # For SliceGPT, ShortGPT and Window Based

  #calibration_dataset = get_calibration(dataset_c4_train, tokenizer, num_samples=num_calibration_samples, seq_len=2048)
  vlcalibration_dataset = get_vlcalibration(dataset_vqa_train, processor, num_samples=num_calibration_samples, seq_len=2048)
  #calibration_dataset_2ssp = calibration_dataset[:num_calibration_samples_2ssp]
  vlcalibration_dataset_2ssp = vlcalibration_dataset[:num_calibration_samples_2ssp]
  ...
  if pruning_method is not None:
    ...
    for target_sparsity in pruning_rates:
      ...
      if pruning_method == "window_based":
        ...
        if model.config.model_type == "qwen2_5_vl":
          model = two_stage_2ssp(model, vlcalibration_dataset_2ssp, dataset_vqa_train, target_sparsity, processor)
        else:
      ...
      save_path = f"/bigdata/qwen2.5-vl-72B-pruned-{sparsity_rate}"
      model.save_pretrained(save_path)


      # 保存 processor（图像处理+文本模板）
