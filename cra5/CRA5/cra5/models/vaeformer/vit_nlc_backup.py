    ...
    def forward(self, x, H, W):
        ...
        # =====================================
            ...
            x = flash_attn_qkvpacked_func(
                qkv, dropout_p=0.0, softmax_scale=self.scale, causal=False
            ).reshape(B, N, C)
            x = x.to(data_type)
        x = self.proj(x)
        return x
...
class PatchEmbed(nn.Module):
    ...
    def __init__(
        self, img_size=224, patch_size=16, patch_stride=16, in_chans=3, embed_dim=768
    ):
        super().__init__()
...
class ViT_Encoder(nn.Module):
    ...
    def __init__(
        ...
        self.num_heads = num_heads
        self.Hp, self.Wp = 0, 0
        self.ori_Hp, self.ori_Hw = (
            (img_size[0] - patch_size[0]) // patch_stride[0] + 1,
            (img_size[1] - patch_size[1]) // patch_stride[1] + 1,
        )

        # Pad-Process-Crop策略：计算padding大小，使Conv2d/ConvTranspose2d完全可逆
        # 对于Conv2d: output_size = (input_size - kernel_size) // stride + 1
        # 对于ConvTranspose2d: output_size = (input_size - 1) * stride + kernel_size
        # 要让它们完全可逆，需要 (input_size - kernel_size) 能被 stride 整除
        # 所以 padding 应该让 (img_size - patch_size) 能被 patch_stride 整除
        diff_h = img_size[0] - patch_size[0]
        diff_w = img_size[1] - patch_size[1]
        pad_h = (patch_stride[0] - (diff_h % patch_stride[0])) % patch_stride[0]
        pad_w = (patch_stride[1] - (diff_w % patch_stride[1])) % patch_stride[1]
        self.pad_h = pad_h
        self.pad_w = pad_w
        self.padded_img_size = (img_size[0] + pad_h, img_size[1] + pad_w)
        global COMPAT
        COMPAT = compat
        ...
        self.z_dim = z_dim
        # 使用padded_img_size初始化patch_embed，确保能被patch_size整除
        ...
        if use_abs_pos_emb:
            ...
            pos_embed = get_2d_sincos_pos_embed(
                self.pos_embed.shape[-1], self.patch_embed.patch_shape, cls_token=False
            )
    ...
    def embedding_forward(self, x, mask=None, **kwargs):
        ...
        x = (
            x + self.pos_embed
        )  # get_abs_pos(pos_embed, False, (self.ori_Hp, self.ori_Hw), patch_shape)
        return x
    ...
    def forward(self, input_var, **kwargs):
        # Pad-Process-Crop: 在输入时进行padding，使尺寸能被patch_size整除
        if self.pad_h > 0 or self.pad_w > 0:
            # padding格式: (pad_left, pad_right, pad_top, pad_bottom)
            input_var = F.pad(
                input_var, (0, self.pad_w, 0, self.pad_h), mode="constant", value=0
            )
...
class ViT_Decoder(nn.Module):
    ...
    def __init__(
        ...
        # Pad-Process-Crop策略：计算padding大小，使Conv2d/ConvTranspose2d完全可逆（与编码器保持一致）
        # 需要让 (img_size - patch_size) 能被 patch_stride 整除
        diff_h = img_size[0] - patch_size[0]
        diff_w = img_size[1] - patch_size[1]
        pad_h = (patch_stride[0] - (diff_h % patch_stride[0])) % patch_stride[0]
        pad_w = (patch_stride[1] - (diff_w % patch_stride[1])) % patch_stride[1]
        ...
        # Hp和Wp应该基于padded尺寸计算，因为实际处理时使用的是padded尺寸
        self.Hp, self.Wp = (
            (self.padded_img_size[0] - patch_size[0]) // patch_stride[0] + 1,
            (self.padded_img_size[1] - patch_size[1]) // patch_stride[1] + 1,
        )
        ...
        # 使用 ConvTranspose2d 作为解码器的最后一层
        # 注意：HyperpriorDecoder 会重写这个属性为 Linear
    ...
    def up_forward(self, x):
        ...
        # 根据 final 层的类型决定如何处理
        if isinstance(self.final, nn.ConvTranspose2d):
            # ConvTranspose2d 需要 (B, C, H, W) 格式
            x = x.permute(0, 3, 1, 2)  # (B, H, W, C) -> (B, C, H, W)
            res = self.final(x)  # (B, out_chans, H_out, W_out)

            # Pad-Process-Crop: 裁剪回原始尺寸（去掉padding部分）
            # 由于输入时做了padding，输出会略大，需要crop回原始尺寸
            if res.shape[2] >= self.img_size[0] and res.shape[3] >= self.img_size[1]:
                res = res[:, :, : self.img_size[0], : self.img_size[1]]
            elif res.shape[2] != self.img_size[0] or res.shape[3] != self.img_size[1]:
                # 如果输出小于目标（罕见情况），使用插值
                res = F.interpolate(
                    res,
            ...
            # Linear 层需要 (B, H, W, C) 格式，然后 rearrange
            # self.Hp和self.Wp已经是基于padded尺寸计算的patch数量
            x = self.final(x)
            res = rearrange(
                x,
                ...
                h=self.Hp,  # 使用实际的patch数量（基于padded尺寸）
                w=self.Wp,
            )
            # Pad-Process-Crop: 裁剪回原始尺寸
            if res.shape[2] >= self.img_size[0] and res.shape[3] >= self.img_size[1]:
                res = res[:, :, : self.img_size[0], : self.img_size[1]]
            elif res.shape[2] != self.img_size[0] or res.shape[3] != self.img_size[1]:
                # 如果输出小于目标（罕见情况），使用插值
...
class HyperpriorDecoder(ViT_Decoder):
    def __init__(
        ...
        self.final = nn.Linear(
            embed_dim, 2 * out_chans * patch_size[-1] * patch_size[-2], bias=False
        )
...
def interpolate_pos_embed(pos_embed_checkpoint, patch_shape, num_extra_tokens):
    ...
    print(
        f"[rank {link.get_rank()}] Position interpolate from {orig_size} to {patch_shape}"
    )
...
def Encoder(
    ...
    if finetune_model is not None:
        ...
        load_state_dict(
            encoder, pretrained_dict_filter, strict=False, logger=dummy_logger
        )

        print(
            "Missing keys: {}".format(
                list(set(model_dict) - set(pretrained_dict_filter))
            )
        )
